...
// Test-only helpers
export function __testOnly_resetLimiter() {
  if (process.env.NODE_ENV === 'test') {
    ipHits.clear()
  }
}

export function rateLimitCheck(key: string, config: LimiterConfig): { allowed: boolean; remaining: number } {
	const now = Date.now()
	const entry = ipHits.get(key) || { count: 0, reset: now + config.windowMs }
	if (now > entry.reset) {
		entry.count = 0
		entry.reset = now + config.windowMs
	}
