...
export async function middleware(request: NextRequest) {
  ...
  // Apply rate limiting to all API routes (BEFORE Auth Guard)
  // This protects the auth/db layer from flooding
  ...
  // Apply admin guard for all admin routes
  if (pathname.startsWith('/admin') || pathname.startsWith('/api/admin')) {
    const adminGuardResult = await adminGuard(request)
    if (adminGuardResult.status !== 200) {
      // If blocked by guard, we still return the response
      // But we verify rate limit headers are present for consistency (and tests)
      const response = adminGuardResult
      const clientIP = getClientIP(request)
      const isAdminRoute = true // We are in the admin block
