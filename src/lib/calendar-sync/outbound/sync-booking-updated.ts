...
export async function syncBookingUpdatedToCalendar(params: {
  booking_id: string;
  provider_id: string;
  ...
  if (!existing) {
    // No mapping exists - fall back to create
    return syncFallbackCreate();
  }
  ...
  if (!adapter.updateEvent) {
    // Fallback: create new event and update mapping deterministically (cancel+recreate could be implemented)
    return syncFallbackCreate();
  }

  try {
    const updated = await adapter.updateEvent({
      id: existing.external_event_id,
      title: payload.title,
      start: payload.start,
      end: payload.end,
      isAllDay: false,
    ...
    await bookingEventRepository.upsertMapping({
      ...
      provider_id,
      calendar_provider,
      external_event_id: (updated && ((updated as any).id || (updated as any).external_id)) || existing.external_event_id,
  ...
  function syncFallbackCreate() {
    // Fall back to create flow by calling created handler logic (not imported to avoid circular deps)
    return { status: 'fallback_create' };
  }
}
