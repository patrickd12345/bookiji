  ...
  try {
    ...
    if (env === 'prod' && !isProduction()) {
      return {
        ...
        success: false,
        message: 'Cannot enable scheduling in prod from non-prod environment'
      }
    }
    ...
    if (flag && flag.value === true) {
      return {
        action_id: 'ENABLE_SCHEDULING',
        success: true,
        message: 'Scheduling already enabled'
      }
    }
    ...
    const reason = context
      ...
      : 'Jarvis SMS command: Enable scheduling'
    ...
    return {
      action_id: 'ENABLE_SCHEDULING',
      success: true,
      message: 'Scheduling enabled successfully'
    }
  } catch (error) {
    return {
      action_id: 'ENABLE_SCHEDULING',
