-- Migration: Ensure simcity_run_events and related tables exist
-- Forward-only fix: Creates tables if they don't exist (idempotent)
-- This addresses cases where migration history shows applied but tables are missing

-- Create simcity_run_events table (Class A: Append-only forensic table)
...
-- Create simcity_run_snapshots table (Class A: Append-only forensic table)
...
-- Create simcity_run_live table (Class B: Operational state)
...
-- Create helper function for metrics
