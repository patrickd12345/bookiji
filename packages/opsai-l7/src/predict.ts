...
export function predictHealthTrend(series: number[]): TrendPrediction {
...
export function predictBookingThroughput(samples: Array<{ timestamp: string; value: number }>): TrendPrediction {
...
export function detectMetricShift(current: number, baseline: number) {
  if (baseline === 0) {
    return { shift: 0, severity: 'low' as const }
  }
