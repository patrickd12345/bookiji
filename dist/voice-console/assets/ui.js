import { OpsAIStream } from './stream.js';
import { speakAnomaly, speakDeployments, speakHealth, speakLastChange, speakSummary } from './voice.js';
...
async function handleHealth() {
    ...
    log({
        title: 'Health ping',
        detail: `Status: ${health.status || health.overall || 'unknown'}`
    });
}
async function handleDeployments() {
    const deployments = await safeJson('/api/ops/deployments', []);
    speakDeployments(Array.isArray(deployments) ? deployments : deployments.deployments);
...
function handleSubscribe() {
    ...
    if (statusEl)
        statusEl.textContent = 'Voice alerts armed via SSE/mock';
