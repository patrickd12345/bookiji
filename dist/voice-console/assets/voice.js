...
export function speakSummary(summary) {
    const deployments = summary.deployments || [];
    const deploymentLine = deployments.length
        ? `${deployments.length} deployments detected. Latest is ${deployments[deployments.length - 1]?.service || 'unknown service'}.`
...
export function speakHealth(health) {
...
export function speakDeployments(deployments) {
...
export function speakLastChange(change) {
    const text = change?.description ||
        'No recent change logs were found. OpsAI will notify you when deployments land.';
    speak(text);
}
export function speakAnomaly(description) {
