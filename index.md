# Documentation Index

This file contains an index of all markdown files in the repository, organized by category.

**Last Updated:** Auto-generated index of all `.md` files

---

## Root Level Files

- `BOOKIJI_FEATURES_FOR_GPT.md`
- `CHANGES.md`
- `CI_CD_SETUP_COMPLETE.md`
- `FIX_SUMMARY.md`
- `GIT_FIX_REPORT.md`
- `LINTER_WARNINGS_EXPLANATION.md`
- `NEW_PROJECT_SETUP_COMPLETE.md`
- `PLAYWRIGHT_CONFIGURATION_SUMMARY.md`
- `PR_NOTES_TEST_STABILITY.md`
- `PRODUCTION_ALIGNMENT_CHECK.md`
- `PROJECT_STATUS.md`
- `PROJECT_TRACKING.md`
- `QA_FREE_TIER_APPROACH.md`
- `QA_MIGRATION_SUMMARY.md`
- `QA_PREVIEW_APPROACH.md`
- `QA_SETUP_COMPLETE.md`
- `QA_SETUP_EXECUTED.md`
- `QA_SETUP_EXECUTION_SUMMARY.md`
- `QA_SETUP_FINAL.md`
- `QA_SIMPLIFIED_APPROACH.md`
- `README.md`
- `README_VERIFICATION_REPORT.md`
- `RESTORE_QUICK_START.md`
- `SIMCITY_V1_ARCHITECTURE.md`
- `STEP_BY_STEP_RESTORE.md`
- `TEST_TRACKER.md`
- `VALIDATION.md`
- `VERCEL_AUDIT_SUMMARY.md`
- `VSCODE_GITHUB_ACTIONS_SETTINGS.md`

---

## Configuration & Setup

### .cursor/
- `.cursor/startup-context.md`

### genome/
- `genome/linter-rules.md`

---

## Applications

### apps/
- `apps/opsai-control-plane/README.md`

---

## Chaos Engineering

### chaos/
- `chaos/OPERATIONAL_STATUS.md`
- `chaos/TIER_A_RESULTS.md`

---

## Documentation

### docs/

#### API Documentation
- `docs/api/API_GUIDE.md`
- `docs/api/AVAILABILITY_SYSTEM_DESIGN.md`
- `docs/api/PROVIDER_RESPONSE_BEST_PRACTICES.md`
- `docs/api/README.md`

#### Bookiji Knowledge Base
- `docs/bookiji-kb/ai-features.md`
- `docs/bookiji-kb/connexion-stripe.md`
- `docs/bookiji-kb/getting-started.md`
- `docs/bookiji-kb/README.md`
- `docs/bookiji-kb/SETUP.md`
- `docs/bookiji-kb/troubleshooting/payment-issues.md`
- `docs/bookiji-kb/vendor/onboarding.md`

#### CI/CD
- `docs/ci-cd/GITHUB_ACTIONS_SETUP.md`
- `docs/ci-cd/PLAYWRIGHT_SETUP.md`

#### Deployment
- `docs/deployment/ANALYTICS_IMPLEMENTATION_COMPLETE.md`
- `docs/deployment/BETA_LAUNCH_CHECKLIST.md`
- `docs/deployment/DATABASE_SETUP.md`
- `docs/deployment/DOMAIN_CONFIGURATION.md`
- `docs/deployment/google-calendar-integration.md`
- `docs/deployment/POST_LAUNCH_ANALYTICS.md`
- `docs/deployment/PRODUCTION_GO_LIVE_CHECKLIST.md`
- `docs/deployment/QA_DEPLOYMENT_GUIDE.md`
- `docs/deployment/QA_ENVIRONMENT_SETUP.md`
- `docs/deployment/README.md`
- `docs/deployment/RESILIENCE_TELEMETRY_SETUP.md`
- `docs/deployment/RESTORE_PAUSED_PROJECT.md`
- `docs/deployment/setup-database.md`
- `docs/deployment/SUPABASE_FUNCTIONS_BUILD_FIX.md`
- `docs/deployment/SUPABASE_MIGRATION_GUIDE.md`
- `docs/deployment/SUPABASE_SETUP.md`

#### Development
- `docs/development/APP_REVIEW_FIXES.md`
- `docs/development/automation-and-scaling-docs.md`
- `docs/development/bookiji-native-calendar-mvp.md`
- `docs/development/BRAND_GUIDELINES.md`
- `docs/development/CI_GUARDS.md`
- `docs/development/COMPREHENSIVE_UI_TESTING_STRATEGY.md`
- `docs/development/CONDITIONAL_BUTTONS_IMPLEMENTATION.md`
- `docs/development/CONTRACT_TESTS.md`
- `docs/development/DATABASE_MANAGEMENT_POLICY.md`
- `docs/development/DATA_POSTURE.md`
- `docs/development/DATA_RECOVERY_RUNBOOK.md`
- `docs/development/DEVELOPER_2_TASKS.md`
- `docs/development/DEVELOPER_3_TASKS.md`
- `docs/development/DISPUTE_NO_SHOW_SYSTEM.md`
- `docs/development/DYNAMIC_BROADCASTING_IMPLEMENTATION_SUMMARY.md`
- `docs/development/FIX_ME.md`
- `docs/development/GENOME_LINTER_GUIDE.md`
- `docs/development/GENOME_LINTER_NEXT_STEPS_PLAN.md`
- `docs/development/GLOBAL_FAIRNESS_SYSTEM.md`
- `docs/development/KB_API_UPGRADE_GUIDE.md`
- `docs/development/KEYWORDS.md`
- `docs/development/MIGRATION_RECOVERY.md`
- `docs/development/MIGRATION_TESTING_STRATEGY.md`
- `docs/development/MODERN_NEXTJS_SETUP.md`
- `docs/development/NEXT_STEPS_IMPLEMENTATION.md`
- `docs/development/NOTIFICATIONS_2_0_AND_I18N_COMPLETION.md`
- `docs/development/NOTIFICATIONS_2_0_I18N_FINAL_SUMMARY.md`
- `docs/development/README.md`
- `docs/development/ROADMAP.md`
- `docs/development/SEO_MANAGER_SYSTEM.md`
- `docs/development/SHADCN_UI_MIGRATION_SUCCESS.md`
- `docs/development/SIMCITY_RUNBOOK.md`
- `docs/development/SIMCITY_TESTING_ENGINE.md`
- `docs/development/SPECIALTY_SYSTEM_DEPLOYMENT.md`
- `docs/development/STARTER_COMMIT_IMPLEMENTATION.md`
- `docs/development/SUPPORT_LOCAL_RUNBOOK.md`
- `docs/development/TEAM_COORDINATION.md`
- `docs/development/TESTING_GUIDE.md`
- `docs/development/TEST_PLAN.md`
- `docs/development/TEST_RESULTS_DASHBOARD.md`
- `docs/development/TRANSLATE_UNTRANSLATED.md`
- `docs/development/TRANSLATION_COMPLETE.md`
- `docs/development/TRANSLATION_GUIDE.md`
- `docs/development/TRANSLATION_TEMPLATE.md`
- `docs/development/WIREFRAMES.md`

#### Features
- `docs/features/simcity-safety.md`

#### Finish Line
- `docs/finish-line/core-booking-flow-validation.md`
- `docs/finish-line/next-phase-plan.md`
- `docs/finish-line/operational_readiness.md`

#### Maintenance
- `docs/maintenance/backup-recovery.md`
- `docs/maintenance/CACHE_INVALIDATION_GUIDE.md`
- `docs/maintenance/error_envelope.md`
- `docs/maintenance/HEALTHAI_SYSTEM.md`
- `docs/maintenance/MAINTENANCE_GUIDE.md`
- `docs/maintenance/PERFORMANCE_OPTIMIZATION_IMPLEMENTATION.md`
- `docs/maintenance/README.md`
- `docs/maintenance/REAL_STATUS_REPORT.md`
- `docs/maintenance/RESILIENCE_TELEMETRY.md`
- `docs/maintenance/RESTORE_DRILL.md`
- `docs/maintenance/SECURITY_NOTICE_ENV_KEYS.md`
- `docs/maintenance/SUPPORT_CRON_SETUP.md`
- `docs/maintenance/UX_RESILIENCE_PATTERNS.md`

#### Operations
- `docs/ops/DEPLOYAI_SYSTEM.md`
- `docs/ops/INCIDENTS_AI.md`
- `docs/ops/LOGSAI.md`
- `docs/ops/PHASE14_BASELINE.md`

#### Support Module
- `docs/support-module/ARCHITECTURE.md`
- `docs/support-module/FINALIZATION_SUMMARY.md`
- `docs/support-module/FREE_TIER_SETUP.md`
- `docs/support-module/LLM_PROVIDER_CONFIG.md`
- `docs/support-module/PRODUCTION_CRAWL_SETUP.md`
- `docs/support-module/PROVIDER_SUMMARY.md`
- `docs/support-module/RECOMMENDED_SETUP.md`
- `docs/support-module/SETUP_AND_TESTING.md`
- `docs/support-module/VERIFICATION_RESULTS.md`
- `docs/support-module/WHY_NOT_GROQ_FOR_BOTH.md`

#### Testing
- `docs/testing/SELECTOR_MAP.md`

#### User Guides
- `docs/user-guides/ADSENSE_APPROVAL_SETUP.md`
- `docs/user-guides/ADSENSE_COMPLIANCE_AUDIT.md`
- `docs/user-guides/BETA_FEEDBACK_SURVEY.md`
- `docs/user-guides/CODEX_ACHIEVEMENTS_SUMMARY.md`
- `docs/user-guides/COMPETITIVE_ANALYSIS.md`
- `docs/user-guides/EMAIL_SYSTEM.md`
- `docs/user-guides/FEATURES_OVERVIEW.md`
- `docs/user-guides/GUIDED_TOURS.md`
- `docs/user-guides/INTERNATIONALIZATION_GUIDE.md`
- `docs/user-guides/INVESTOR_PITCH_SLIDES.md`
- `docs/user-guides/landing-and-onboarding-copy.md`
- `docs/user-guides/LANDING_PAGE.md`
- `docs/user-guides/LOYALTY_FEATURES_PLAN.md`
- `docs/user-guides/OFFLINE_SUPPORT_AND_MOBILE_APPS.md`
- `docs/user-guides/PATENT_OUTLINE.md`
- `docs/user-guides/PERFORMANCE_CONFIGURATION.md`
- `docs/user-guides/PPP_SYSTEM.md`
- `docs/user-guides/PRIVACY_POLICY.md`
- `docs/user-guides/PROVIDER_ONBOARDING.md`
- `docs/user-guides/QUICK_SETUP.md`
- `docs/user-guides/README.md`
- `docs/user-guides/RELEASE_NOTES_ADMIN_COCKPIT.md`
- `docs/user-guides/SUGGESTED_TERMS_COPY.md`
- `docs/user-guides/VENDOR_FEE_SYSTEM.md`

#### Other Documentation
- `docs/BOOKIJI_CONTINUITY_KERNEL.md`
- `docs/BUG_REPORT_VERIFICATION.md`
- `docs/BUGS_TO_FIX.md`
- `docs/CHANGELOG.md`
- `docs/CI_IMPROVEMENTS.md`
- `docs/CI_LEVEL2_SUMMARY.md`
- `docs/CI_PLAYBOOK.md`
- `docs/FIXES_APPLIED.md`
- `docs/genome-overview.md`
- `docs/how-to-update-genome.md`
- `docs/IMMUTABLE_DEPLOYS.md`
- `docs/LEVEL2_ENHANCEMENTS.md`
- `docs/LEVEL3_ENHANCEMENTS.md`
- `docs/LEVEL4_ENHANCEMENTS.md`
- `docs/LEVEL5_ENHANCEMENTS.md`
- `docs/MONOREPO_SETUP.md`
- `docs/NEEDS_HELP.md`
- `docs/NIGHTLY_VERIFICATION_REPORT.md`
- `docs/ORGANIZATION_SUMMARY.md`
- `docs/P2-notes.md`
- `docs/README.md`
- `docs/SEO_MANAGEMENT_MODEL.md`
- `docs/simcity-phase3.md`
- `docs/simcity-ui-foundation.md`
- `docs/SLO_DASHBOARD.md`
- `docs/test-coverage-verification.md`
- `docs/VERIFICATION_SUMMARY.md`

---

## Issues

### issues/
- `issues/analytics-system-metrics.md`
- `issues/booking-cancel-notifications.md`
- `issues/vendor-registration-db.md`

---

## Operations

### ops/
- `ops/drills/rollback-drill-2024-08-24.md`
- `ops/phase14/phase14-baseline-runlog.template.md`

---

## Packages

### packages/
- `packages/opsai-helpdesk/README.md`
- `packages/opsai-l7/README.md`
- `packages/opsai-sdk/README.md`

---

## Reference

### reference/
- `reference/verification-rules.md`

---

## Scripts

### scripts/
- `scripts/recovery/README.md`
- `scripts/recovery/report.template.md`

---

## Source Code

### src/
- `src/components/README.md`

---

## Supabase

### supabase/
- `supabase/migrations/_hold/README.md`

---

## Tests

### tests/
- `tests/Pre-Phase14-Legal-Baseline.md`
- `tests/README.md`

---

## Notes

- This index is automatically maintained. When creating a new `.md` file, ensure it is added to this index.
- Files are organized by their directory structure and purpose.
- For detailed information about each file, refer to the file itself.
